...
```{r}
long.df <- long.c %>% 
  select(adhd, frameref, all_of(analysis.cols), consc) %>% 
  pivot_longer(cols=all_of(c(analysis.cols, "consc")))
```

```{r}
p.adhd <- long.df %>% 
  mutate(adhd = factor(adhd, levels = c("Non-ADHD", "ADHD")),
         name = str_replace(name, "consc", "Conscientiousness"),
         name = str_replace_all(name, "[.]", "-"),
         name = str_to_title(name)) %>% 
  ggplot(aes(x=adhd, y = value, group=adhd))+
  geom_boxplot(aes(colour=adhd), position="dodge2")+
  facet_wrap(~name)+
  theme_bw()+
  theme(panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
  ...
  xlab("")+
  ylab("Score")
cowplot::save_plot(paste0(output,"adhd_nadhd_comparison.png"), p.adhd)
```
...
p.for <- long.df %>% 
  mutate(frameref = str_to_title(frameref),
         ...
         name = str_replace_all(name, "[.]", "-"),
         name = str_to_title(name)) %>% 
  ggplot(aes(x=frameref, y = value, group=frameref))+
  geom_boxplot(aes(colour=frameref), position="dodge2")+
  facet_wrap(~name)+
  theme_bw()+
  theme(panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
  ...
  xlab("")+
  ylab("Score")
cowplot::save_plot(paste0(output,"work_gen_comparison.png"), p.for)
```
...
work.cols <- facet_items %>% filter(str_starts(facet, "W")==T)
work.cols <- work.cols$new_varname
```



## Measurement Invariance: Work FOR

```{r}
#build formulas for the CFA
selfeff <- paste0(facet_items$new_varname[facet_items$facet=="WC1"],
                 collapse=" + ")
order <- paste0(facet_items$new_varname[facet_items$facet=="WC2"],
                 collapse=" + ")
duty <- paste0(facet_items$new_varname[facet_items$facet=="WC3"],
                 collapse=" + ")
achieve <- paste0(facet_items$new_varname[facet_items$facet=="WC4"],
                 collapse=" + ")
selfdisc <- paste0(facet_items$new_varname[facet_items$facet=="WC5"],
                 collapse=" + ")
caution <- paste0(facet_items$new_varname[facet_items$facet=="WC6"],
                 collapse=" + ")
```

```{r}
#specify the factor structure
facstruc <- paste0("w.selfeff =~ ", selfeff, "\n",
                  "w.order =~ ", order, "\n",
                  "w.duty =~ ", duty, "\n",
                  "w.achieve =~ ", achieve, "\n",
                  "w.selfdisc =~ " , selfdisc, "\n",
...
#metric model definitely fits worse than configural
summary(metric.config.comp)
```

```{r}
metric_lavtest <- lavTestScore(metric)
metric_lavtest <- data.frame(metric_lavtest$uni)
naughty <- metric_lavtest %>% filter(p.value < .05)
ptabs <- parTable(metric)
naughty_items <- ptabs %>%
...
cat(paste0("The modification index tests the improvement in model fit if the equality constraints for individual parameters were released. This test suggested that releasing equality constraints on the loadings of ",
           'one item from the achievement-striving scale ("', 
           naughty_item_wording$clean_var_desc[1], 
...
#savve empty df to avoid appending issue
write.csv(data.frame(), 
          paste0(output, "metric.config.comp.csv"))
...
```{r}
naughty.metric <- naughty_items$eq[!duplicated(naughty_items$eq)]
partial.metric <- cfa(facstruc,
              data=df,
              group="adhd",
              missing="fiml",
              estimator="ML",
              group.equal=c("loadings"),
              group.partial = naughty.metric)
summary(partial.metric,fit.measures=TRUE)

anova(partial.metric, config)
partial.metric.config.comp <- compareFit(config, partial.metric)
#save model comparison to local excel file
write.csv(data.frame(), 
          paste0(output, "partial.metric.config.comp.csv"))
saveFile(partial.metric.config.comp,
         tableFormat = T,
         fit.measures = c("chisq", "df", "pvalue", "cfi", "tli",
                          "rmsea", "rmsea.ci.lower", "rmsea.ci.upper",
              "srmr"),
         file=paste0(output,"partial.metric.config.comp.csv"), 
         writeArgs = list(append = F, sep=","))

summary(partial.metric.config.comp)#looks like the simpler model (metric invariance) fits better than the complex model (configural invariance)
fitm.metric <- t(data.frame(fitMeasures(partial.metric,
            c("chisq", "df", "pvalue", "cfi", "tli", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper",
              "srmr"))))
fitm.metric <- apply(fitm.metric, 2, function(x) number(x, accuracy = .01))#store fit in dataframe
fitm.metric["Model"] <- "M2a Partial Metric Invariance"
fitm.config<- rbind(fitm.config, fitm.metric)#bind m1 fit with this model's fit
...
anova(scalar, partial.metric)# the ANOVA is significant, which suggests noninvariance
scalar.metric.comp <- compareFit(partial.metric, scalar)
...
cat(paste0("The modification index tests the improvement in model fit if the equality constraints for individual parameters were released. This test suggested that releasing equality constraints on the intercepts of ",
           'two items from the self-efficacy scale ("', 
           naughty_item_wording$clean_var_desc[1], 
           '" and "',
           naughty_item_wording$clean_var_desc[2], 
           '"), ', 
           'two items from the orderliness scale ("',
           naughty_item_wording$clean_var_desc[3], 
           '" and "',
           naughty_item_wording$clean_var_desc[4], 
           '"), two items from the self-discipline scale ("',
           naughty_item_wording$clean_var_desc[5], 
           '" and "',
           naughty_item_wording$clean_var_desc[6], 
           '"), and one item from the cautiousness scale ("',
           naughty_item_wording$clean_var_desc[7], 
           '") would improve model fit. ')
    )
...
partial.scalar <- cfa(facstruc,
              ...
              group.equal=c("loadings",
                            "intercepts"),
              group.partial = c(naughty_items$eq[!duplicated(naughty_items$eq)],
                                naughty.metric))
partial.scalar.comp <- compareFit(partial.metric, partial.scalar)

#save the model comparisons locally
write.csv(data.frame(),
          paste0(output,"partial.scalar.comp.csv"))
saveFile(partial.scalar.comp,
         tableFormat = T,
         fit.measures = c("chisq", "df", "pvalue", "cfi",
                          "tli", "rmsea", "rmsea.ci.lower", "rmsea.ci.upper","srmr"),
         file=paste0(output,"partial.scalar.comp.csv"),
         writeArgs = list(append=F,sep=","))
...
#save local
write.csv(fitm.config, paste0(output,"work_partial_invariance_model.csv"))
