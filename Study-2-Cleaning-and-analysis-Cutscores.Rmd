...
Create ADHD grouping variable based on both self-identification and clinical cut scores:

```{r}
df$adhd.screen <- ifelse(df$adhd_yn=="Yes",
                         "ADHD",
                         ifelse(df$adhd_yn=="No",
                                "Non-ADHD",
                                ifelse((df$adhd_yn=="Not sure"&df$asrs_sum>=14),
                                       "ADHD", "Non-ADHD")))
with(df, table(adhd, adhd.screen))
#assign new adhd.screen variable to the adhd variable for remaining analyses
df$adhd <- ifelse(df$adhd_yn=="Yes",
                         "ADHD",
                         ifelse(df$adhd_yn=="No",
                                "Non-ADHD",
                                ifelse((df$adhd_yn=="Not sure"&df$asrs_sum>=14),
...
write.csv(threeway.wide, paste0(output, "screen_3way_inrxn_pres_order.csv"))
```
...
write.xlsx(pw_piv, paste0(output, "screen_rstatix_pairwise_results.xlsx"))
